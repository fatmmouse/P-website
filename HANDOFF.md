...
- **8 个页面**：首页 / 文章×3 / about / for-agents / projects / 404
  - 文章 = 2 篇小红书连载（`kind: serial`）+ 1 篇学术特别刊（`kind: essay`，《将军底头》拉康式分析）
...
- **域名 + 部署**：定了改 `site/astro.config.mjs` 的 `site` 字段；Astro 纯静态，`dist/` 直接传 Cloudflare Pages / EdgeOne
